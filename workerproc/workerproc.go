...
func StartWorkerProcessor(processorFailed chan<- struct{}, cfg *server.Config, myMetrics *metrics.Set) *WorkerProcessor {
	...
	go func() {
		...
		for request := range reqChan {
			...
			//Compared to my initial implementation, modifying local copies of buckets hurts locality - rather than working with
			// same stack-local value in all requests, now we use that stack location to make a unique copy for each request,
			// so, many hardware optimizations cannot be applied anymore.
			// It might make sense to associate dedicated worker with each
			...
			// I did some *load tests* however, and they don't show significant differences. Actually, they weren't stable enough
